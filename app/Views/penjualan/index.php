...
<script>
   ...
   function tableLoad() {
      ...
      $.ajax({
         type: "GET",
         url: BASE_URL + "/penjualan/readPenjualan",
         success: function(data) {
            ...
            $.each(data, function(i, val) {
               ...
               t += '<td>' + tglIndo(val.jual_tgl) + '</td>';
               t += '<td>' + val.jual_desc + '</td>';
               t += '<td>' + formatRupiah(val.jual_total) + '</td>';
               t += `<td class="text-center">
                     <div class="btn-group">
                           <button class="btn btn-sm btn-info item_detail" title="Detail" data-id="` + val.jual_id + `"><i class="fas fa-info"></i> Detail</button>
                           <button class="btn btn-sm btn-warning item_edit" title="Edit" data-id="` + val.jual_id + `"><i class="fas fa-edit"></i></button>
                           <button class="btn btn-sm btn-danger item_hapus" title="Hapus" data-id="` + val.jual_id + `" data-name="` + val.jual_desc + `"><i class="fas fa-trash"></i></button>
                        </div>
                     </td>`;
               t += '</tr>';
         ...
         $.ajax({
            ...
            success: function(res) {
               if (res.status == 200) {
                  ...
                  $("#modal-detail div.modal-header h4.modal-title").html("Detail Penjualan");
                  $("#modal-detail div.invoice-number").html(`<p>` + res.data.jual_tgl + `</p>`);
                  $("#modal-detail div.tgl-detail p").html(tglIndo(res.data.jual_tgl));
                  $("#modal-detail div.desc-detail p").html(res.data.jual_desc);
                  $("#modal-detail div.invoice-detail-value").html(formatRupiah(res.data.jual_total));
                  var no = 1;
   ...
   $(document).off("click", "table#tb-data button.item_edit")
      .on("click", "table#tb-data button.item_edit", function(e) {
         e.preventDefault();
         let url = BASE_URL + "/penjualan/edit/" + $(this).attr("data-id");
         location.href = url;
      });
      ...
      .on("click", "table#tb-data button.item_hapus", function(e) {
         ...
         swal({
            ...
            if (result) {
               $.ajax({
                  type: "POST",
                  url: BASE_URL + "/penjualan/deletePenjualan",
                  data: {
                     id: id
                  },
                  dataType: "json",
   ...
   $(document).off("click", "button.item_tambah")
      .on("click", "button.item_tambah", function(e) {
         e.preventDefault();
         let url = BASE_URL + "/penjualan/add";
         location.href = url;
      });
